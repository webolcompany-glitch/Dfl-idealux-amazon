...
qta_min = 0
maggiorazione_idealux = 0.0

if fornitore == "Idealux":
    qta_min = st.sidebar.number_input("Quantità minima (Idealux)", min_value=0, value=1, step=1)
    # Nuovo parametro esclusivo per Idealux
    maggiorazione_idealux = st.sidebar.number_input(
        "Maggiorazione prodotti Idealux fascia 0-20 €", min_value=0.0, value=5.0, step=1.0
    )

# --- Funzioni costo spedizione ---
def costo_spedizione_IT(peso):
    if peso <= 2: return 5.71
    elif peso <= 3: return 5.81
    elif peso <= 5: return 7.02
    elif peso <= 10: return 9.66
    elif peso <= 25: return 13.92
    elif peso <= 49: return 21.73
    return None

def costo_spedizione_DE(peso):
    if peso <= 1: return 8.97
    elif peso <= 2: return 9.86
    elif peso <= 3: return 10.74
    elif peso <= 5: return 11.72
    elif peso <= 10: return 13.03
    elif peso <= 15: return 15.02
    elif peso <= 20: return 16.90
    elif peso <= 25: return 21.09
    elif peso <= 30: return 22.98
    return None

def costo_spedizione_FR(peso):
    if peso <= 1: return 9.98
    elif peso <= 2: return 10.99
    elif peso <= 3: return 12.02
    elif peso <= 5: return 13.15
    elif peso <= 10: return 15.88
    elif peso <= 15: return 18.01
    elif peso <= 20: return 20.03
    elif peso <= 25: return 24.83
    elif peso <= 30: return 26.85
    return None

def costo_spedizione_NL(peso):
    if peso <= 1: return 9.26
    elif peso <= 2: return 10.16
    elif peso <= 3: return 12.02
    elif peso <= 5: return 12.11
    elif peso <= 10: return 13.97
    elif peso <= 15: return 16.68
    elif peso <= 20: return 19.34
    elif peso <= 25: return 23.61
    elif peso <= 30: return 26.27
    return None

def costo_spedizione_ES(peso):
    if peso <= 1: return 10.11
    elif peso <= 2: return 11.14
    elif peso <= 3: return 12.02
    elif peso <= 5: return 14.46
    elif peso <= 10: return 16.90
    elif peso <= 15: return 19.43
    elif peso <= 20: return 21.79
    elif peso <= 25: return 28.79
    elif peso <= 30: return 31.13
    return None

def costo_spedizione_BE(peso):
    if peso <= 1: return 10.72
    elif peso <= 2: return 11.77
    elif peso <= 3: return 12.82
    elif peso <= 5: return 14.01
    elif peso <= 10: return 15.87
    elif peso <= 15: return 18.62
    elif peso <= 20: return 21.28
    elif peso <= 25: return 26.22
    elif peso <= 30: return 28.88
    return None
...
if uploaded_file:
    try:
        ...
        if fornitore == "DFL":
            ...
            st.info(f"🔍 Maggiorazione Idealux (0-20€): **{maggiorazione_idealux:.2f} €** (Ricarico % ignorato)")
        ...
        # --- LOGICA DI PREZZO DIFFERENZIATA ---
        if fornitore == "Idealux":
            def calcola_prezzo_idealux(prezzo_pubblico):
                if pd.notna(prezzo_pubblico) and 0 <= prezzo_pubblico <= 20:
                    return prezzo_pubblico + maggiorazione_idealux
                return prezzo_pubblico
            
            # Ignora il markup, usa il Prezzo al Pubblico + eventuale maggiorazione
            df["PrezzoFinale"] = df["PrezzoAlPubblico"].apply(calcola_prezzo_idealux)
        else:
            # Per DFL la logica resta invariata
            df["PrezzoAcquisto"] = prezzo_netto + df["CostoSpedizione"]
            df["PrezzoFinale"] = df["PrezzoAcquisto"] * moltiplicatore
        ...
        if mercato == "Italia":
            ...
            # Applica il Prezzo Finale già calcolato con la logica corretta
            prezzo_vendita_listino = df_listino["PrezzoFinale"]

            df_listino_output = pd.DataFrame()
            df_listino_output["SKU"] = f"{fornitore}_" + cod_art.loc[df_listino.index].astype(str)
            ...
            df_listino_output["Prezzo EUR (Vendita su Amazon, IT)"] = prezzo_vendita_listino.round(2)
        ...
        # Applica il Prezzo Finale già calcolato
        prezzo_vendita_aggiorna = df_aggiorna["PrezzoFinale"]

        df_aggiorna_output = pd.DataFrame()
        df_aggiorna_output["SKU"] = f"{fornitore}_" + cod_art.loc[df_aggiorna.index].astype(str)
        df_aggiorna_output["price"] = prezzo_vendita_aggiorna.round(2)
